...
define([
  ...
  var optimize,
    cssImportRegExp = /\@import\s+(url\()?\s*([^);]+)\s*(\))?([\w, ]*)(;)?/gi,
  ...
  optimize = {
      ...
      if (optimizerName && optimizerName !== "none") {
        ...
        try {
          if (config.preserveLicenseComments) {
            ...
            try {
              licenseContents = parse.getLicenseComments(
                fileName,
                fileContents,
              );
            } catch (e) {
              throw new Error(
                "Cannot parse file: " +
                  fileName +
                  " for comments. Skipping it. Error is:\n" +
                  e.toString(),
              );
            }
          }
          ...
          fileContents =
            licenseContents +
            optFunc(fileName, fileContents, outFileName, keepLines, optConfig);
          if (
    ...
    babel: function (content, options = {}) {
      try {
    ...
    cssFile: function (fileName, outFileName, config) {
      ...
      var originalFileContents = file.readFile(fileName),
        ...
        fileContents = flat.skippedList.length
          ? originalFileContents
          : flat.fileContents,
        startIndex,
        endIndex,
      ...
      if (config.optimizeCss.indexOf("standard") !== -1) {
        ...
        if (fileList) {
          for (i = 0; i < fileList.length; i++) {
            fileName = fileList[i];
            logger.trace(
              "Optimizing (" + config.optimizeCss + ") CSS file: " + fileName,
            );
    ...
    optimizers: {
      uglify: function (
        ...
        if (
          ...
          (outFileName || config._buildSourceMap)
        ) {
