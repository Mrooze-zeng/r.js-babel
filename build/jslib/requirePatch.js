...
define([
  ...
  "prim",
  "optimize",
], function (file, pragma, parse, lang, logger, commonJs, prim, optimize) {
  var allowRun = true,
  ...
  return function () {
    ...
    function normalizeUrlWithBase(context, moduleName, url) {
      ...
      if (
        require.jsExtRegExp.test(moduleName) &&
      ...
      if (name === "_") {
          ...
          if (context.config.wrapShim) {
            fn = function () {
              ...
              if (value.init) {
                str +=
                  "(" +
                  value.init.toString().replace(useStrictRegExp, "") +
        ...
        context.enable = function (depMap, parent) {
          ...
          if (mod && !mod.defined) {
            ...
            (getOwn(needFullExec, id) && falseProp(fullExec, id)) ||
            (parentId &&
              getOwn(needFullExec, parentId) &&
        ...
        context.load = function (moduleName, url) {
          ...
          if (require._isSupportedBuildUrl(url)) {
            ...
            prim()
              .start(function () {
                if (
                  hasProp(require._cachedFileContents, url) &&
                  ...
                  if (
                    ...
                    require._cachedDefinesRequireUrls[url]
                  ) {
                  ...
                  return require
                    ...
                    .then(function (text) {
                      ...
                      if (context.config.onBuildRead) {
                        contents = context.config.onBuildRead(
                          moduleName,
                          url,
                          contents,
                        );
                    ...
                    .then(function () {
                      if (hasProp(context.plugins, moduleName)) {
                        ...
                        if (pluginBuilderMatch) {
                          ...
                          builderName = context.makeModuleMap(
                            pluginBuilderMatch[3],
                            context.makeModuleMap(moduleName),
                            null,
                            true,
                          ).id;
                          return require._cacheReadAsync(
        ...
        moduleProto.init = function (depMaps) {
          if (context.needFullExec[this.map.id]) {
            lang.each(
              depMaps,
              lang.bind(this, function (depMap) {
                if (typeof depMap === "string") {
                  depMap = context.makeModuleMap(
                    depMap,
        ...
        moduleProto.callPlugin = function () {
          ...
          if (
            falseProp(context.fullExec, pluginId) &&
