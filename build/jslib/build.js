...
define(function (require) {
    ...
    if (sourceMapGenerator) {
      ...
      if (parts.length === 1) {
        ...
        if (resourceIsModuleIdRegExp.test(resourcePath)) {
          sourceMapPath =
            build.makeRelativeFilePath(refPath, require.toUrl(resourcePath)) +
            "!" +
            pluginId;
        } else {
          sourceMapPath = path;
  ...
  build = function (args) {
    ...
    return prim()
      ...
      .then(null, function (e) {
        ...
        if (
          typeof args === "string" &&
          args.indexOf("stacktrace=true") !== -1
        ) {
          errorMsg += "\n" + errorStack;
        } else {
    ...
    return prim()
      .start(function () {
          ...
          if (config.appDir) {
            ...
            for (prop in paths) {
              if (hasProp(paths, prop)) {
                buildPaths[prop] = paths[prop].replace(
                  config.appDir,
                  config.dir,
                );
              }
        ...
        if (config.out) {
          ...
          require(config);
          if (!config.cssIn) {
            config.modules[0]._buildPath =
              typeof config.out === "function" ? "FUNCTION" : config.out;
          }
          ...
          if (modules) {
            modules.forEach(function (module) {
              if (module.name) {
                ...
                if (
                  module._buildPath === module._sourcePath &&
                ...
                if (
                  !module.create &&
                  (!config.rawText ||
                    !lang.hasProp(config.rawText, module.name))
                ) {
          ...
          actions = modules.map(function (module, i) {
            return function () {
              ...
              config._buildPathToModuleIndex[
                file.normalize(module._buildPath)
              ] = i;

              //Call require to calculate dependencies.
              return build
                .traceDependencies(module, config, baseConfig)
                .then(function (layer) {
                  module.layer = layer;
                });
            };
          ...
          actions = modules.map(function (module) {
            return function () {
              if (module.exclude) {
                ...
                return prim.serial(
                  module.exclude.map(function (exclude, i) {
                    return function () {
          ...
          return prim.serial(
            modules.map(function (module) {
              return function () {
                ...
                return build
                  .flattenModule(module, module.layer, config)
                  .then(function (builtModule) {
                    ...
                    if (module._buildPath === "FUNCTION") {
                      ...
                      if (builtModule.sourceMap) {
                        ...
                        file.saveUtf8File(
                          module._buildPath + ".map",
                          builtModule.sourceMap,
                        );
                      }
      ...
      .then(function () {
          ...
          modules.forEach(function (module) {
            ...
            if (finalPath !== "FUNCTION") {
              ...
              if (bundlesConfigOutFile) {
                ...
                var parsedIds = parse.getAllNamedDefines(
                  bundleContents,
                  excludeMap,
                );
        ...
        if (config.out && !config.cssIn) {
          ...
          if (fileName === "FUNCTION") {
            ...
            config.modules[0]._buildText = optimize.js(
          ...
          fileNames.forEach(function (fileName) {
            ...
            moduleIndex =
              moduleIndex === 0 || moduleIndex > 0 ? moduleIndex : -1;
            ...
            if (
              ...
              if (moduleIndex === -1) {
                if (config.onBuildRead) {
                  fileContents = config.onBuildRead(
                    moduleName,
                    fileName,
                    fileContents,
                  );
                ...
                if (config.normalizeDirDefines === "all") {
                  fileContents = build.toTransport(
                    config.namespace,
                    null,
                    fileName,
                    fileContents,
                ...
                if (config.onBuildWrite) {
                  fileContents = config.onBuildWrite(
                    moduleName,
                    fileName,
                    fileContents,
                  );
              ...
              override =
                moduleIndex > -1 ? config.modules[moduleIndex].override : null;
              if (override) {
                ...
                if (falseProp(pluginProcessed, moduleMap.id)) {
                  ...
                  if (plugin && plugin.writeFile) {
                    plugin.writeFile(
                      ...
                      moduleMap.name,
                      require,
                      makeWriteFile(config.namespace),
                      context.config,
                    );
                  }
        ...
        if (config.cssIn) {
          buildFileContents += optimize.cssFile(
            config.cssIn,
            config.out,
            config,
          ).buildText;
        ...
        if (typeof config.out === "function") {
          config.out(
            config.modules[0]._buildText,
            config.modules[0]._buildSourceMap,
          );
        }
  ...
  build.makeAbsConfig = function (config, absFilePath) {
    ...
    build.makeAbsObject(
      config.out === "stdout" ? ["cssIn"] : ["out", "cssIn"],
      config,
      absFilePath,
    );
  ...
  function flattenWrapFile(config, keyName, absFilePath) {
    ...
    if (typeof wrap[keyName] !== "string" && wrap[keyFileName]) {
      ...
      wrap[keyMapName] = [
        function (fileContents, cfg, sourceMapGenerator) {
          var absPath = build.makeAbsPath(
            "config-wrap-" + keyName + "-default.js",
            absFilePath,
          );
          return appendToFileContents(
            fileContents,
      ...
      throw new Error(
        "wrap." + keyName + " or wrap." + keyFileName + " malformed",
      );
    }
  }
  ...
  build.createConfig = function (cfg) {
    ...
    mainConfigFile =
      ...
      (buildFileConfig && buildFileConfig.mainConfigFile);
    if (mainConfigFile) {
    ...
    if (config.bundlesConfigOutFile) {
      if (!config.dir) {
        throw new Error(
          "bundlesConfigOutFile can only be used with optimizations " +
            'that use "dir".',
        );
      }
      config.bundlesConfigOutFile = build.makeAbsPath(
        config.bundlesConfigOutFile,
        config.dir,
      );
    }
    ...
    if (config.out && config.out === "stdout") {
      config.out = function (content) {
        var e = env.get();
        if (e === "rhino") {
          var out = new java.io.PrintStream(
    ...
    if (config.main) {
      throw new Error(
        '"main" passed as an option, but the ' +
    ...
    if (config.cssIn) {
      if (config.dir || config.appDir) {
        throw new Error(
          "cssIn is only for the output of single file " +
    ...
    if (!config.out && !config.dir) {
      throw new Error(
        'Missing either an "out" or "dir" config value. ' +
    ...
    if (config.appDir && config.out) {
      throw new Error(
        '"appDir" is not compatible with "out". Use "dir" ' +
    ...
    if (config.out && config.dir) {
      throw new Error(
        'The "out" and "dir" options are incompatible.' +
    ...
    if (config.dir) {
      ...
      if (
        ...
        (config.dir === config.baseUrl ||
          ...
          (config.baseUrl &&
            build
              .makeRelativeFilePath(config.dir, config.baseUrl)
              .indexOf("..") !== 0) ||
          (config.appDir &&
            build
              .makeRelativeFilePath(config.dir, config.appDir)
              .indexOf("..") !== 0))
      ) {
        throw new Error(
          '"dir" is set to a parent or same directory as' +
    ...
    if (config.insertRequire && !lang.isArray(config.insertRequire)) {
      throw new Error(
        "insertRequire should be a list of module IDs" +
    ...
    if (config.generateSourceMaps) {
      if (
        ...
        !(config.optimize === "none" || config.optimize === "uglify")
      ) {
        throw new Error(
          "Cannot use preserveLicenseComments and " +
    ...
    if ((config.name || config.include) && !config.modules) {
      ...
      throw new Error(
        'If the "modules" option is used, then there ' +
      ...
      throw new Error(
        '"name" and "modules" options are incompatible. ' +
    ...
    if (config.modules && config.modules.length) {
      config.modules.forEach(function (mod) {
        if (lang.isArray(mod) || typeof mod === "string" || !mod) {
          throw new Error(
            "modules config item is malformed: it should" +
  ...
  build.traceDependencies = function (module, config, baseLoaderConfig) {
    ...
    include = include.concat(
      module.name && !module.create ? [module.name] : [],
    );
    if (module.include) {
    ...
    if (module.override) {
      if (baseLoaderConfig) {
        override = build.createOverrideConfig(
          baseLoaderConfig,
          module.override,
        );
      } else {
  ...
  build.flattenModule = function (module, layer, config) {
    ...
    return prim()
      .start(function () {
        ...
        stubModulesByName =
          (module.stubModules && module.stubModules._byName) || {};
        ...
        return prim
          .serial(
            layer.buildFilePaths.map(function (path) {
              return function () {
                var singleContents = "";
                ...
                return prim()
                  .start(function () {
                    ...
                    builder =
                      parts.prefix && getOwn(context.defined, parts.prefix);
                    if (builder) {
                      if (
                        builder.onLayerEnd &&
                        falseProp(onLayerEndAdded, parts.prefix)
                      ) {
                      ...
                      if (builder.write) {
                        writeApi = function (input) {
                          ...
                          if (config.onBuildWrite) {
                            singleContents = config.onBuildWrite(
                              moduleName,
                              path,
                              singleContents,
                            );
                          ...
                          singleContents +=
                            ...
                            addSemiColon(
                              build.toTransport(
                                ...
                                {
                                  useSourceUrl:
                                    layer.context.config.useSourceUrl,
                                },
                              ),
                              config,
                            );
                      ...
                      return prim()
                        ...
                        .then(function (text) {
                          ...
                          if (
                            config.cjsTranslate &&
                            (!config.shim ||
                              !lang.hasProp(config.shim, moduleName))
                          ) {
                            currContents = commonJs.convert(path, currContents);
                          }

                          if (config.onBuildRead) {
                            currContents = config.onBuildRead(
                              moduleName,
                              path,
                              currContents,
                            );
                          ...
                          if (packageName) {
                            hasPackageName =
                              packageName ===
                              parse.getNamedDefine(currContents);
                          }
                          ...
                          if (config.onBuildWrite) {
                            currContents = config.onBuildWrite(
                              moduleName,
                              path,
                              currContents,
                            );
                  ...
                  .then(function () {
                    ...
                    if (
                      moduleName &&
                      falseProp(layer.modulesWithNames, moduleName) &&
                      !config.skipModuleInsertion
                    ) {
                      shim =
                        config.shim &&
                        (getOwn(config.shim, moduleName) ||
                    ...
                    fileContents = appendToFileContents(
                      fileContents,
                      singleContents,
                      path,
                      config,
